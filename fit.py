...
# resize the mask for faster clustering and fitting
config.resize_factor = 0.3
...
def draw_history(blend_img, history, history_cnt, history_record, history_platenum):
    ...
    for i, plate in enumerate(history):
        ...
        leftshift = 480
        ph, pw = plate.shape[:2]
        if 130+step*i+ph >= blend_img.shape[0]:
            continue
        blend_img[130+step*i:130+step*i+ph,w-leftshift:w-leftshift+pw,:] = plate
        text = '违章记录:第%d帧' %history_record[i]
        blend_img = cv2ImgAddText(blend_img, text, w-leftshift+pw+10,130+step*i, textColor=(0, 0, 0),\
                    textSize=30, font_path="./LiHeiPro.ttf")        
        if history_platenum[i] != ' ':
            text = '车牌识别:'+ history_platenum[i]
            blend_img = cv2ImgAddText(blend_img, text, w-leftshift+pw+10,130+step*i+35, textColor=(0, 0, 0),\
                        textSize=30, font_path="./LiHeiPro.ttf")
...
if __name__ == "__main__":
    # save_dir = '1a_res_final'
    # save_dir = '3d_res_final'
    save_dir = '1a_res_final'
    ...
    # get the all boxes results (include vehicle box, plate box, vehicle head box, vehicle tail box)
    json_path = 'boxandattrs/1aattrs.json'
    boxes_results = []
    ...
    # get lane segmentation results
    with open('1a_mask_res.pkl', 'rb') as f:
        mask_results = cPickle.load(f)['all_seg_results']
        mask_results = [_['seg_results'] for _ in mask_results]
    ...
    for cnt in range(num_img):
        ...
        # if cnt < 112:
            # continue
        img = mask_results[cnt]

        im_path = os.path.join(img_dir, '%08d.jpg' %(cnt+1))
        ...
        blend_img = cv2.addWeighted(raw_img, 0.8, lane_img, 0.3, gamma=0)
        ...
        # raw img --> resize to smaller size --> cluster and fit lane --> resize to original size
        ...
        blend_img = cv2ImgAddText(blend_img, title, 20,20, textColor=(0, 0, 0),\
                        textSize=60, font_path="./LiHeiPro.ttf")
        title = '压线车辆:'
        blend_img = cv2ImgAddText(blend_img, title, w-300,70, textColor=(255, 0, 0),\
                        textSize=50, font_path="./LiHeiPro.ttf")

        # collect the solid lanes (cls==2)
        lanes = []
        for cls in group_res:
            # cls 1 means dotted line, cls 2 means solid line
            if cls == 1:
                continue
            # print('----cls %d----' %cls)
        ...
        # judge whether the vehicle cross the solid lane
        for box in tail_box:
            bottom_line = [box[0],box[3],box[2],box[3]]  # (x1,y2,x2,y2)
            
            for lane in lanes:
                input1 = bottom_line + lane
                input2 = lane + bottom_line
                if (is_cross(*input1) and is_cross(*input2)):
                    flag = 20
                    text = '压实线'
                    print(text)
                    blend_img = cv2ImgAddText(blend_img, text, int((box[0]+box[2])/2),int(box[1]), textColor=(255, 0, 255),\
                                 textSize=30, font_path="./LiHeiPro.ttf")
